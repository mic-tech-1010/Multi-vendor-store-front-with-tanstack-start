...
function Header({session}: {session: UserSession}) {
    ...
    return (
        ...
        <header className='overflow-hidden'>
            ...
            <nav
                aria-label='main'
                className="bg-primary border-b border-border max-tablet:flex max-tablet:flex-wrap max-tablet:justify-between max-tablet:pt-1"
            >

                {/* top Nav Bar started here (the div is flatten with display of contents for max width of 760px) */}

                <div className='flex max-tablet:contents max-tablet:flex-wrap justify-between gap-1 xl:gap-2 items-end w-full text-foreground max-w-full p-2.5 bg-primary'>
                    ...
                    <div className="flex items-center gap-3 tablet:contents mr-2.5">
                        ...
                        {isDesktop && (
                            <a
                                href=""
                                className="text-left leading-[1.1] text-link hover:outline-solid! hover:outline-offset-3! hover:outline-ring! hover:outline-1"
                            >
                                <span className="text-[13px] text-link/90">Returns</span>
                                <span className="block text-base font-medium text-link">&amp; Orders</span>
                            </a>
                        )}
                        ...
                        <Link
                            to="/cart"
                            className="flex items-end gap-1 text-left leading-[1.1] text-link hover:outline-solid! hover:outline-offset-3! hover:outline-ring! hover:outline-1 pr-0 tablet:pr-3"
                        >
                ...
                <div className='flex max-tablet:order-2 gap-[3ch] justifty-start items-center bg-secondary text-foreground max-lg:overflow-auto scrollbar-hide '>
                    ...
                    <menu className="flex items-center px-1 py-2.5 gap-4 text-nowrap">
                        {menuItems.map(({ label, href }, index) => (
                            <li key={index}>
                                <a
                                    href={href}
                                    className='text-base text-white! p-0 hover:outline-solid! hover:outline-offset-3! hover:outline-ring! hover:outline-1 focus-visible:border-none rounded-none! focus-visible:outline-none focus-visible:shadow-[0_0_0_2px_hsl(var(--ring)),0_0_0_3px_hsl(var(--background))] transition-none text-[clamp(0.8125rem,0.625rem+0.417vw,0.9375rem)]'
                                >
                                    {label}
                                </a>
                            </li>
