                ...
                <ul>
                    <CarouselContent className="">
                        ...
                        {section.items.map((item: any) => (
                            <CarouselItem key={item.id} className="basis-1/3 md:basis-1/4 lg:basis-1/6 aspect-square max-h-35 sm:max-h-48 h-full">
...
function GridItem({ item }: { item: any }) {
    if (item.product) {
        return <ProductLink product={item.product} />;
    }

    if (item.category) {
        return <CategoryLink category={item.category} />;
    }
...
function CategoryLink({ category }: { category: any }) {
    return (
        <Link to="/product-details" className="flex h-full">
            <img src={category.bannerUrl} alt={category.slug} className="object-contain max-h-full w-full" />
        </Link>
    );
}
